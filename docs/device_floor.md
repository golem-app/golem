...
- **Install floor: iOS 17.0** (`IPHONEOS_DEPLOYMENT_TARGET`, all twelve Runner
  configurations). It is the MLX carrier's own floor — see below — and
  nothing lower is honest: the carrier is not linked at launch but
  `dlopen`ed by the first native call, which is the launch probe, so an iOS
  15 or 16 install would start and die before its first frame rather than
  be refused by the store. `com.apple.developer.kernel.increased-memory-limit`
  (added with #62, so multi-gigabyte weights plus an 8192-token KV cache fit
  under the jetsam ceiling) needs only 15.0 and still holds. Decided in
...
- **Store gate: A12.** `UIRequiredDeviceCapabilities` names `arm64` and
  ...
  only expanding device requirements in an update. Every A12-or-later
  iPhone runs iOS 17, so the store gate and the install floor describe the
  same set of phones.
- **MLX engine floor: iOS 17** (the `InfernoMLXCarrier` SwiftPM platform
  ...
  builds compose MLX (ADR 0012), so this floor always binds on iOS and is
  why the install floor above is 17. The llama.cpp-Metal shim, reachable by
  dart-define, carries a lower floor of its own that never decides anything.
