...
class ModelController extends _$ModelController {
  ...
  Future<void> selectBackend(BackendId backend) async {
    if (state.requireValue.runtime == RuntimePhase.loading) return;
    state = AsyncData(
  ...
  Future<void> pauseMlx() async {
    _operationEpoch++;
    state = AsyncData(
      await ref.read(modelManagementRepositoryProvider).pauseMlx(),
    );
  }
  ...
  Future<void> importTurboFieldfare() async {
    final epoch = ++_operationEpoch;
    await for (final value
        in ref.read(modelManagementRepositoryProvider).importTurboFieldfare()) {
      if (epoch != _operationEpoch) return;
      state = AsyncData(value);
    }
  }
  ...
  Future<void> toggleRuntime() async {
    ...
    state = AsyncData(
      current.runtime == RuntimePhase.loaded
          ? await repository.unloadRuntime()
          : await repository.loadRuntime(),
    );
  }
}
...
class BenchmarkController extends _$BenchmarkController {
  ...
  void selectCase(String caseId) => state = state.copyWith(caseId: caseId);
